...
def serve_code(location, token, crd): return send(token, location, search_res(location, menu_choose(dust_weather(weather(crd)[0], weather(crd)[1]))), dust_weather(weather(crd)[0], weather(crd)[1]))
